...
#' PopHumanAnalysis(genes=mygenes , pops=c("CEU"), recomb=TRUE, bins=3, test="eMKT")
#' 
#' @import utils
#' @import stats
#'
...
PopHumanAnalysis <- function(genes=c("gene1","gene2","..."), pops=c("pop1","pop2","..."), cutoff=0.05, recomb=TRUE/FALSE, bins=0, test=c("standardMKT","eMKT","FWW","aMKT"), xlow=0, xhigh=1, plot=FALSE) { 
	...
	stop("Parameter test must be one of the following: standardMKT, eMKT, FWW, aMKT")
	}
	if (length(test) > 1) {
